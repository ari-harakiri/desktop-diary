...
// DesktopDiary Paint v0.1 — self-contained application object.
  ...
  (function(){
    ...
    var kobaTaskbarClickTimer=null;
    function minimizeKobaFromTaskbar(){
      kobaTaskbarClickTimer=null;
      ...
      toggle.title='Double-click to search with Koba';
    }
    function searchWithKobaFromTaskbar(){
      clearTimeout(kobaTaskbarClickTimer);kobaTaskbarClickTimer=null;
      document.body.classList.remove('koba-minimized');
      ...
      toggle.title='Click once to minimize Koba · double-click to search';
    }
    toggle.title='Click once to minimize Koba · double-click to search';
    toggle.setAttribute('aria-label','Click once to minimize Koba; double-click to search');
    toggle.addEventListener('click',function(e){
      if(e.detail>1)return;
      clearTimeout(kobaTaskbarClickTimer);
      kobaTaskbarClickTimer=setTimeout(minimizeKobaFromTaskbar,240);
    });
    toggle.addEventListener('dblclick',function(e){e.preventDefault();e.stopPropagation();searchWithKobaFromTaskbar();});
  ...
  function openOnlineDtdPostWindow(){
    ...
    createWindow({title:'DtD Post Mail',extraClass:'dtd-win',initialLeft:74,initialTop:72,bodyHtml:'<div class="win-body">'+bar()+'<div style="padding:28px;text-align:center">Connecting to DtD Post Mail…</div></div>',type:'dtdmail',onClose:function(){detachPigeonFromMailWindow();pigeonIcon().classList.remove('mail-window-open');if(pigeonComposingActive)endPigeonComposing(false);},onMount:function(el){
      ...
      function openMessage(id){
        ...
        var receivedTime=folder==='inbox'?(m.deliver_on||m.created_at):m.created_at;
        main.innerHTML='<div class="dtd-meta"><button class="dtd-profile-link" title="View '+escapeHtml(profileHandle)+'\'s profile">'+escapeHtml(initials)+'</button><b>'+escapeHtml(m.message_subject||'(no subject)')+'</b><br>From: '+escapeHtml(address(m.from_handle))+'<br>To: '+escapeHtml(address(m.to_handle))+'<br>'+(folder==='scheduled'?'Delivery: ':'')+escapeHtml(scrapbookDate(new Date(receivedTime).getTime()))+'</div><div class="dtd-message">'+escapeHtml(m.message_body||'')+'</div><div class="dtd-inline-reply" style="display:none"><textarea class="dtd-inline-reply-body" placeholder="Write a fresh letter back…"></textarea><div class="dtd-inline-reply-actions"><span class="dtd-inline-reply-note">The previous letter will not be included.</span><span><button class="btn dtd-reply-cancel">Cancel</button> <button class="btn dtd-reply-send">Send Reply</button></span></div><div class="signon-error dtd-reply-error"></div></div><div style="padding:7px;border-top:1px solid #ccc;display:flex;justify-content:space-between"><button class="btn dtd-back">Back</button><span><button class="btn dtd-save-diary">Save to Diary</button> <button class="btn dtd-save-contact">Save E-Buddy</button> <button class="btn dtd-reply">Reply</button> <button class="btn dtd-delete-mail">Delete</button></span></div>';
