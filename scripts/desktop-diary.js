...
(function(){
  ...
  // === BEGIN scripts/services/supabase.js ===
  ...
  // === END scripts/services/supabase.js ===

  // === BEGIN scripts/core/state.js ===
  var state = {
    ...
    notebook: {                // single digital notebook (device-local): { id, title, text, createdAt, updatedAt, order }
      pages: [],
      currentPageId: null
    },
  ...
  function normalizeState(){
    ...
    // migrate old sticky-note data into the new notebook model
    var legacySticky = Array.isArray(state.stickyNotes) ? state.stickyNotes : [];
    if(!state.notebook || typeof state.notebook !== 'object') state.notebook = { pages: [], currentPageId: null };
    if(!Array.isArray(state.notebook.pages) || !state.notebook.pages.length){
      state.notebook.pages = legacySticky.filter(function(note){
        ...
        return {
          ...
          order: Number.isFinite(Number(note.order)) ? Number(note.order) : (idx + 1)
        };
      });
    }
    state.notebook.pages = Array.isArray(state.notebook.pages) ? state.notebook.pages.filter(function(page){
      ...
      return {
        ...
        createdAt: Number.isFinite(Number(page.createdAt)) ? Number(page.createdAt) : Date.now() + i,
        updatedAt: Number.isFinite(Number(page.updatedAt)) ? Number(page.updatedAt) : Date.now() + i,
        order: Number.isFinite(Number(page.order)) ? Number(page.order) : (i + 1)
      };
    }).sort(function(a, b){
    ...
    state.notebook.currentPageId = (typeof state.notebook.currentPageId === 'string') ? state.notebook.currentPageId : null;
    if(state.notebook.currentPageId && !state.notebook.pages.some(function(page){ return page.id === state.notebook.currentPageId; })){
      state.notebook.currentPageId = null;
    }
    if(!state.notebook.currentPageId) state.notebook.currentPageId = state.notebook.pages[0].id;
    state.stickyNotes = [];
    ...
    if(!Array.isArray(state.anonymousTips)) state.anonymousTips = [];
    state.anonymousTips = state.anonymousTips
      .map(function(tip){
        if(!tip || typeof tip !== 'object') return null;
        var topic = (typeof tip.topic === 'string' ? tip.topic : 'General').trim().slice(0, 40) || 'General';
        var text = (typeof tip.text === 'string' ? tip.text : '').trim().slice(0, 1500);
        var ts = Number.isFinite(Number(tip.ts)) ? Number(tip.ts) : Date.now();
        return {
          id: typeof tip.id === 'string' ? tip.id : ('tip-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2)),
          topic: topic,
          text: text,
          ts: ts
        };
      })
      .filter(function(tip){ return tip && tip.text; })
  ...
  // === END scripts/core/state.js ===

  // === BEGIN scripts/features/sticky-notes-and-background.js ===
  ...
  // === END scripts/features/sticky-notes-and-background.js ===

  // === BEGIN scripts/features/desktop-personalization.js ===
  ...
  var mailboxIcon=document.getElementById('dtd-mailbox-icon'),mailboxClickTimer=null;
  ...
  (function(){
    ...
    if(suggestionIcon){
      suggestionIcon.addEventListener('click',function(e){
        e.stopPropagation();
        openSuggestionBoxWindow();
      });
      suggestionIcon.addEventListener('dblclick',function(e){
        e.preventDefault();
        e.stopPropagation();
        openSuggestionBoxWindow();
      });
      suggestionIcon.addEventListener('keydown',function(e){
        if(e.key!=='Enter'&&e.key!==' ')return;
        e.preventDefault();
        e.stopPropagation();
        openSuggestionBoxWindow();
    ...
    trashArt.addEventListener('animationend',function(e){if(e.animationName==='trash-desktop-wiggle')trashArt.classList.remove('is-wiggling');});
    ...
    trashIcon.addEventListener('dblclick',function(e){
      ...
      clearTimeout(trashClickTimer);trashClickTimer=0;
      trashArt.classList.remove('is-wiggling');
      openTrashWindow();
    });
  ...
  (function(){
    ...
    var arinetArt=arinetIcon&&arinetIcon.querySelector('.arinet-desktop-icon-art');
    var arinetSpinning=false,arinetOpenAfterSpin=false,arinetSpinFallback=0;
    function finishAriNetSpin(){
      if(!arinetSpinning)return;
      clearTimeout(arinetSpinFallback);arinetSpinFallback=0;
      arinetArt.classList.remove('is-spinning');
      arinetSpinning=false;
      if(arinetOpenAfterSpin){arinetOpenAfterSpin=false;openAriNetWindow();}
    }
    function startAriNetSpin(){
      if(!arinetArt){openAriNetWindow();return;}
      arinetSpinning=true;
      arinetArt.classList.remove('is-spinning');
    ...
    arinetArt.addEventListener('animationend',function(e){if(e.animationName==='arinet-desktop-spin')finishAriNetSpin();});
    arinetIcon.addEventListener('click',function(e){
      e.stopPropagation();
      if(arinetSpinning){arinetOpenAfterSpin=true;return;}
      startAriNetSpin();
    });
  ...
  // === END scripts/features/desktop-personalization.js ===

  // === BEGIN scripts/features/paint.js ===
...
// DesktopDiary Paint v0.1 — self-contained application object.
  ...
  // === END scripts/features/paint.js ===

  // === BEGIN scripts/core/persistence.js ===
  // ---------------- persistence ----------------
  ...
  // === BEGIN scripts/services/firebase.js ===
  ...
  // === END scripts/services/firebase.js ===

  // === BEGIN scripts/core/media-and-helpers.js ===
  // ---------------- sound ----------------
  function getCtx(){
    if(!window._actx){
  ...
  var DEFAULT_BG_B64 = 'assets/embedded/dreamy-green-hills-background.png';
  ...
  function refreshRecoveredContent(item){
    refreshTrashIcon();
    if(item.type==='sticky' || item.type==='notebook-page'){
      if(!state.notebook || !Array.isArray(state.notebook.pages)) return;
      if(item.type==='notebook-page' && item.data){
        if(!state.notebook.pages.some(function(page){return page.id===item.data.id;})){
          var sorted = state.notebook.pages.slice().sort(function(a,b){ return (a.order||0)-(b.order||0); });
          sorted.push(item.data);
          sorted = sorted.map(function(page,index){ page.order = index+1; return page; });
          state.notebook.pages = sorted;
        }
      } else if(item.type==='sticky' && item.data && item.data.id){
        var legacy = {
          id: item.data.id,
          ...
          createdAt: Date.now(),
          updatedAt: Date.now(),
          order: (state.notebook.pages.length || 0) + 1
        };
        if(!state.notebook.pages.some(function(page){return page.id===legacy.id;}))state.notebook.pages.push(legacy);
      }
    }
  ...
  function restoreTrashItem(item){
    if(item.type==='sticky' || item.type==='notebook-page'){
      if(!state.notebook || !Array.isArray(state.notebook.pages)) state.notebook = { pages: [], currentPageId: null };
      if(item.type==='notebook-page'){
        if(!(state.notebook.pages||[]).some(function(n){return n.id===item.data.id;})){
          state.notebook.pages.push(item.data);
        }
      } else {
        if(!(state.notebook.pages||[]).some(function(n){return n.id===item.data.id;})){
          state.notebook.pages.push({
            id: item.data.id || uid(),
            title: item.data.title || '',
            text: item.data.text || '',
      ...
      state.notebook.pages = (state.notebook.pages||[]).map(function(page){return page;}).map(function(page,idx){page.order=idx+1;return page;});
      state.notebook.currentPageId = item.data && item.data.id ? item.data.id : state.notebook.currentPageId;
  ...
  function trashTypeName(type){
    return type==='notebook-page' ? 'Notebook Page' : (type==='sticky' ? 'Sticky Note' : (type==='buddy' ? 'Buddy' : (type==='buddyEntry' ? 'Diary Entry' : 'Private Diary Entry')));
  }
  ...
  // === END scripts/core/media-and-helpers.js ===

  // === BEGIN scripts/features/sign-on.js ===
  // ================= SIGN ON =================
  ...
  // === END scripts/features/sign-on.js ===

  // === BEGIN scripts/features/buddy-list.js ===
  // ================= BUDDY LIST =================
  ...
  function signOff(closePage){
    ...
    var signOffSoundSequenceMs = 470;
  ...
  // === END scripts/features/buddy-list.js ===

  // === BEGIN scripts/features/window-system.js ===
  ...
  // === END scripts/features/window-system.js ===

  // === BEGIN scripts/shared/rich-compose.js ===
  // ================= IM WINDOW =================
  var WRITING_PROMPTS = [
  ...
  // === BEGIN scripts/features/instant-messages.js ===
  ...
  // === END scripts/features/instant-messages.js ===

  // === BEGIN scripts/features/search-and-companion.js ===
  ...
  // === END scripts/features/search-and-companion.js ===
  ...
  // === END scripts/shared/entry-filters.js ===

  // === BEGIN scripts/features/scrapbook.js ===
  // ================= SCRAPBOOK =================
  ...
  // === BEGIN scripts/features/new-message.js ===
  // ================= NEW MESSAGE WINDOW =================
  ...
  // === BEGIN scripts/features/buddies.js ===
  ...
  // === END scripts/features/buddies.js ===

  // === BEGIN scripts/features/status.js ===
  // ================= STATUS WINDOWS =================
  ...
  // === END scripts/features/status.js ===

  // === BEGIN scripts/features/logs.js ===
  ...
  // === END scripts/features/logs.js ===

  // === BEGIN scripts/features/pigeon-delivery.js ===
  ...
  // === BEGIN scripts/shared/post-mail-helpers.js ===
  ...
  // === END scripts/shared/post-mail-helpers.js ===

  // === BEGIN scripts/features/post-mail-offline.js ===
  function openDtdPostWindow(){
    ...
    createWindow({title:'DtD Post Mail',extraClass:'dtd-win',initialLeft:74,initialTop:72,bodyHtml:body,type:'dtdmail',onClose:function(){detachPigeonFromMailWindow();pigeonIcon().classList.remove('mail-window-open');if(pigeonComposingActive)endPigeonComposing(false);},onMount:function(el){
      ...
      function showCompose(to,subject){
        if(!getSupabaseSession()){
          endPigeonComposing(false);
          main.innerHTML='<div class="dtd-heading">New Message</div><div class="dtd-compose" style="text-align:center"><p style="font-size:11px;line-height:1.5;text-align:left;margin:0 0 12px">Oops! Please make an account to send outgoing mail!</p><span class="forgot-link" id="dtd-create-account">Create an account</span><div class="dtd-compose-actions"><button class="btn dtd-compose-cancel">Back</button></div></div>';
          main.querySelector('#dtd-create-account').onclick=function(){openCreateAccountWindow('connect');};
  ...
  // === END scripts/features/post-mail-offline.js ===
  ...
  function openOnlineDtdPostWindow(){
    ...
    createWindow({title:'DtD Post Mail',extraClass:'dtd-win',initialLeft:74,initialTop:72,bodyHtml:'<div class="win-body">'+bar()+'<div style="padding:28px;text-align:center">Connecting to DtD Post Mail…</div></div>',type:'dtdmail',onClose:function(){detachPigeonFromMailWindow();pigeonIcon().classList.remove('mail-window-open');if(pigeonComposingActive)endPigeonComposing(false);},onMount:function(el){
      ...
      function loadProfile(){return ensureSupabaseSession().then(function(session){return supabaseRestRequest('dtd_profiles?select=handle,display_name&user_id=eq.'+encodeURIComponent(session.user.id)+'&limit=1');}).then(function(rows){return rows&&rows[0]||null;});}
      ...
      function loadFolder(quiet){
        ...
        supabaseRpc('get_dtd_mailbox',{box_name:serverBox,client_timezone:dtdClientTimezone()}).then(function(rows){var today=todayString();rows=rows||[];if(folder==='inbox')rows=updatePigeonOnlineInbox(rows,today);else if(folder==='sent')rows=rows.filter(function(m){return !(m.from_handle===profile.handle&&m.to_handle===profile.handle&&m.deliver_on>String(m.created_at).slice(0,10));});currentRows=rows;renderRows();}).catch(function(err){main.innerHTML='<div class="dtd-empty">'+escapeHtml(err.message)+'</div>';});
      }
      ...
      function openMessage(id){
        ...
        if(priv){
          ...
          main.innerHTML='<div class="dtd-meta"><b>'+escapeHtml(priv.subject||'(no subject)')+'</b><br>From: You<br>To: '+privateRecipientHtml+'<br>'+escapeHtml(scrapbookDate(new Date(receivedTime).getTime()))+'</div>'+disclaimer+'<div class="dtd-message">'+escapeHtml(m.message_body||'')+'</div><div style="padding:7px;border-top:1px solid #ccc;display:flex;justify-content:space-between"><button class="btn dtd-back">Back</button><span><button class="btn dtd-save-diary">Save to Diary</button> <button class="btn dtd-delete-mail">Delete</button></span></div>';
          main.querySelector('.dtd-back').onclick=renderRows;
          main.querySelector('.dtd-save-diary').onclick=function(){saveDtdMailToDiary('online:'+m.message_id,priv.subject,'You',priv.displayRecipient,m.message_body,m.created_at);};
        ...
        if(reply){
          if(!replyBox){
            ...
            if(replySend)replySend.onclick=function(){var bodyField=main.querySelector('.dtd-inline-reply-body'),body=bodyField?bodyField.value:'',text=body.trim(),sendButton=replySend,errorBox=main.querySelector('.dtd-reply-error');if(!text){if(errorBox)errorBox.textContent='Write your reply first.';return;}sendButton.disabled=true;supabaseRpc('send_dtd_message',{recipient_handle:profileHandle,message_subject:m.message_subject||'',message_body:body,delivery_date:nextDeliveryDate(),client_timezone:dtdClientTimezone()}).then(function(){trackDtdUsage('letter_sent');endPigeonComposing(true);openInfoWindow('Your reply is sealed and will be delivered at the next 8:00am.');setFolder('sent');}).catch(function(err){if(errorBox)errorBox.textContent=err.message;sendButton.disabled=false;});};
          }
        }
      }
      ...
      function showCompose(to,subject){
        ...
        var toEl=main.querySelector('#dtd-to'),bodyEl=main.querySelector('#dtd-body'),send=main.querySelector('.dtd-send');toEl.oninput=function(){toEl.value=toEl.value.toLowerCase().replace(/[^a-z0-9._-]/g,'');};main.querySelector('.dtd-compose-cancel').onclick=loadFolder;send.onclick=function(){var recipient=recipientHandle(toEl.value),text=bodyEl.value,date=main.querySelector('#dtd-send-date').value||nextDeliveryDate();if(!recipient){main.querySelector('.dtd-error').textContent='Enter your friend\'s username.';return;}if(!text.trim()){main.querySelector('.dtd-error').textContent='Write a message first.';return;}send.disabled=true;supabaseRpc('send_dtd_message',{recipient_handle:recipient,message_subject:main.querySelector('#dtd-subject').value.trim(),message_body:text,delivery_date:date,client_timezone:dtdClientTimezone()}).then(function(){trackDtdUsage('letter_sent');endPigeonComposing(true);setFolder('sent');}).catch(function(err){main.querySelector('.dtd-error').textContent=err.message;send.disabled=false;});};setTimeout(function(){(to?bodyEl:toEl).focus();},20);
      }
  ...
  // === END scripts/features/post-mail-online.js ===

  // === BEGIN scripts/features/arinet.js ===
  ...
  // === END scripts/features/arinet.js ===

  // === BEGIN scripts/features/help-and-admin.js ===
  // ================= HELP WINDOW =================
  ...
  function dtdAnonymousTipToken(){
    var key='dtd-anonymous-tip-client-v1',token='';
    try{token=localStorage.getItem(key)||'';}catch(e){}
    if(/^[A-Za-z0-9_-]{24,100}$/.test(token))return token;
    if(window.crypto&&typeof window.crypto.randomUUID==='function')token=window.crypto.randomUUID();
    else{
      var alphabet='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-';
      for(var i=0;i<40;i++)token+=alphabet.charAt(Math.floor(Math.random()*alphabet.length));
    }
  ...
  function openSuggestionBoxWindow(){
    var existing=openWindows.find(function(w){return w.type==='suggestion-box';});
    if(existing){focusWindow(existing.id);return;}
    var body=
      '<div class="win-body suggestion-win">' +
        '<div class="suggestion-intro">Send an anonymous idea, bug note, or request to DesktopDiary. Your email, screen name, account ID, and diary contents are not included.</div>' +
        '<div class="suggestion-field"><label for="suggestion-topic">Topic (optional)</label><input id="suggestion-topic" maxlength="40" placeholder="e.g. UI polish, bug, feature idea"></div>' +
        '<div class="suggestion-field"><label for="suggestion-text">Your anonymous tip</label><textarea id="suggestion-text" maxlength="1500" placeholder="Type your tip here…"></textarea></div>' +
        '<div class="suggestion-honeypot" aria-hidden="true"><label for="suggestion-website">Website</label><input id="suggestion-website" type="text" tabindex="-1" autocomplete="off"></div>' +
        '<div class="suggestion-actions">' +
          '<div class="suggestion-status" aria-live="polite"></div>' +
          '<div class="suggestion-buttons">' +
            '<button class="btn suggestion-submit" type="button">Send Tip</button>' +
            '<button class="btn" id="suggestion-close" type="button">Close</button>' +
          '</div>' +
        '</div>' +
      '</div>';
    createWindow({title:'Suggestion Box',extraClass:'suggest-win',bodyHtml:body,type:'suggestion-box',onMount:function(el,id){
      var topicInput=el.querySelector('#suggestion-topic');
      var textInput=el.querySelector('#suggestion-text');
      var websiteInput=el.querySelector('#suggestion-website');
      var submitBtn=el.querySelector('.suggestion-submit');
      var status=el.querySelector('.suggestion-status');
      var closeBtn=el.querySelector('#suggestion-close');
      function setStatus(message,color){
        status.style.color=color||'#555';
        status.textContent=message;
      }
      submitBtn.onclick=function(){
        var topic=(topicInput.value||'').trim().slice(0,40) || 'General';
        var text=(textInput.value||'').trim().slice(0,1500);
        if(!text){setStatus('Please add a message before sending.','#b03');return;}
        submitBtn.disabled=true;
        setStatus('Sending…','#666');
        supabasePublicRpc('submit_dtd_anonymous_tip',{
          requested_topic:topic,
          requested_message:text,
          requested_device_class:dtdUsageDeviceClass(),
          requested_client_token:dtdAnonymousTipToken(),
          requested_website:websiteInput.value||''
        }).then(function(){
          topicInput.value='';
          textInput.value='';
          setStatus('Thanks — your anonymous tip was sent.','green');
          submitBtn.disabled=false;
        }).catch(function(err){
          submitBtn.disabled=false;
          setStatus('Unable to send right now. Please try again later.','#b03');
        });
      };
    ...
    refreshAdminAccess().then(function(allowed){
      ...
      return loadDtdSiteContent().then(function(){
        var body='<div class="win-body admin-panel">'+
          ...
          '<div class="admin-tabs" role="tablist"><button class="admin-tab active" type="button" role="tab" aria-selected="true" data-admin-tab="content">Help &amp; Install</button><button class="admin-tab" type="button" role="tab" aria-selected="false" data-admin-tab="suggestions">Suggestions</button><button class="admin-tab" type="button" role="tab" aria-selected="false" data-admin-tab="usage">Usage Statistics</button></div>'+
          ...
          '</section>'+
          '<section class="admin-tab-panel" data-admin-panel="suggestions"><div class="admin-suggestion-toolbar"><div class="admin-suggestion-status" aria-live="polite"></div><button class="btn admin-suggestion-refresh" type="button">Refresh</button></div><div class="admin-suggestion-list"></div></section>'+
        ...
        createWindow({title:'Administration',extraClass:'admin-win',bodyHtml:body,type:'admin',constructionBar:false,onMount:function(el){
          ...
          var suggestionsLoaded=false,suggestionsLoading=false,suggestionStatus=el.querySelector('.admin-suggestion-status'),suggestionList=el.querySelector('.admin-suggestion-list'),suggestionRefresh=el.querySelector('.admin-suggestion-refresh');
          function suggestionDate(value){var date=new Date(value);return Number.isFinite(date.getTime())?date.toLocaleString():'Unknown date';}
          function renderSuggestions(rows){
            rows=Array.isArray(rows)?rows:[];
            suggestionList.innerHTML=rows.length?rows.map(function(tip){
              return '<article class="admin-suggestion-card"><div class="admin-suggestion-head"><b>'+escapeHtml(tip.topic||'General')+'</b><span class="admin-suggestion-meta">'+escapeHtml(suggestionDate(tip.created_at))+' · '+escapeHtml(tip.device_class||'desktop')+'</span></div><div class="admin-suggestion-message">'+escapeHtml(tip.message||'')+'</div></article>';
            }).join(''):'<div class="usage-empty">No suggestions yet.</div>';
          }
          function loadSuggestions(){
            if(suggestionsLoading)return;
            suggestionsLoading=true;suggestionRefresh.disabled=true;suggestionStatus.style.color='#555';suggestionStatus.textContent='Loading suggestions…';
            supabaseRpc('get_dtd_anonymous_tips',{requested_limit:100}).then(function(rows){
              suggestionsLoaded=true;suggestionStatus.textContent='';renderSuggestions(rows);
            }).catch(function(err){
              suggestionStatus.style.color='#c0392b';suggestionStatus.textContent=err.message||'Suggestions are unavailable. Run the anonymous tips SQL setup first.';suggestionList.innerHTML='';
            }).then(function(){suggestionsLoading=false;suggestionRefresh.disabled=false;});
          }
          suggestionRefresh.addEventListener('click',loadSuggestions);
          ...
          el.querySelectorAll('[data-admin-tab]').forEach(function(tab){tab.addEventListener('click',function(){var name=tab.dataset.adminTab;el.querySelectorAll('[data-admin-tab]').forEach(function(button){var active=button===tab;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));});el.querySelectorAll('[data-admin-panel]').forEach(function(panel){panel.classList.toggle('active',panel.dataset.adminPanel===name);});if(name==='suggestions'&&!suggestionsLoaded)loadSuggestions();if(name==='usage'&&!usageLoaded)loadUsage();});});
  ...
  // === END scripts/features/help-and-admin.js ===

  // === BEGIN scripts/features/cloud-sync.js ===
  // ================= CLOUD SYNC WINDOW =================
  function openCloudSyncWindow(){
    if(!requireOnlineAccount('Cloud Sync')) return;
  ...
  // === END scripts/features/cloud-sync.js ===

  // === BEGIN scripts/features/profiles.js ===
  ...
  // === BEGIN scripts/features/profile-diary.js ===
  ...
  // === BEGIN scripts/features/taskbar-clock.js ===
  // ================= TASKBAR CLOCK =================
  ...
  // === END scripts/features/taskbar-clock.js ===

  // === BEGIN scripts/features/calendar.js ===
  // ================= CALENDAR =================
  ...
  if(!calendarPopup || !calTitle || !calGrid || !calPrev || !calNext) return;
  ...
  function renderCalendarPopup(){
    ...
    calendarPopup.querySelectorAll('.cal-cell[data-day]').forEach(function(cell){
  ...
  // === END scripts/features/calendar.js ===

  // === BEGIN scripts/core/boot.js ===
  // ================= BOOT =================
