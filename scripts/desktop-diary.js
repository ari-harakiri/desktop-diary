  ...
  function initializeDesktopIconDragging(){
    ...
    icons.forEach(function(icon){
      ...
      if(icon.id==='desktop-companion' && !saved){
        saved = {
          x: icon.offsetWidth ? Math.max(0, window.innerWidth - icon.offsetWidth) : 0,
          y: icon.offsetHeight ? Math.max(0, window.innerHeight - TASKBAR_H - icon.offsetHeight) : 0,
          overBuddyList: false
        };
...
// DesktopDiary Paint v0.1 — self-contained application object.
  var DesktopDiaryPaint = {
    ...
    createWindow({title:'DesktopDiary Paint v2.3',extraClass:'paint-win',bodyHtml:body,type:'paint',onMount:function(el){
      ...
      function buttons(){setMenuEnabled('undo',!!hist.length);setMenuEnabled('redo',!!redo.length);setMenuEnabled('editText',!!lastText);['cut','copy','delete','crop','resizeSel'].forEach(function(a){setMenuEnabled(a,!!selection);});setMenuEnabled('paste',!!clipboard);}
      ...
      el.querySelectorAll('[data-a]').forEach(function(b){b.onclick=function(){if(b.classList.contains('disabled'))return;var a=b.dataset.a;if(a==='new')newImg();if(a==='import')fi.click();if(a==='export')save();if(a==='undo')undoIt();if(a==='redo')redoIt();if(a==='editText')editLastText();if(a==='cut')cutSelection();if(a==='copy')copySelection();if(a==='paste')pasteSelection();if(a==='delete')deleteSelection();if(a==='crop')cropSelection();if(a==='resizeSel')resizeSelection();if(a==='clear')clear();if(a==='fh')transform('h');if(a==='fv')transform('v');if(a==='rl')transform('l');if(a==='rr')transform('r');if(a==='resize')resize();if(a==='saveDiary')saveToDiary();if(a==='setbg'){state.background={color:'',gradient:'',image:cv.toDataURL('image/png')};applyBackground();saveState();msg.textContent='Set as desktop background';}if(a==='about')openInfoWindow('DesktopDiary Paint v2.3 — send drawings into the profile Diary post editor.');};});
      el.tabIndex=0;el.onkeydown=function(e){var k=e.key.toLowerCase(),mod=e.ctrlKey||e.metaKey;if(mod&&k==='n'){e.preventDefault();newImg();}if(mod&&k==='z'){e.preventDefault();e.shiftKey?redoIt():undoIt();}if(mod&&k==='y'){e.preventDefault();redoIt();}if(mod&&k==='s'){e.preventDefault();save();}if(mod&&k==='o'){e.preventDefault();fi.click();}if(mod&&k==='c'){e.preventDefault();copySelection();}if(mod&&k==='x'){e.preventDefault();cutSelection();}if(mod&&k==='v'){e.preventDefault();pasteSelection();}if(k==='delete'||k==='backspace'){if(selection){e.preventDefault();deleteSelection();}}if(k==='escape')clearSelection();};
  ...
  function pushToCloud(){
    ...
    var meta = {
      ...
      mailContacts: Array.isArray(state.mail && state.mail.contacts) ? state.mail.contacts : [],
  ...
  function pullFromCloud(){
    ...
    return db.collection('users').doc(uid).collection('diary').doc('meta').get().then(function(snap){
      ...
      return Promise.all(fetches.concat([bgFetch])).then(function(){
        ...
        if(Array.isArray(meta.mailContacts)) state.mail.contacts = meta.mailContacts.slice();
  ...
  (function(){
    ...
    FetchToyController.prototype.finishFetch=function(direction){
      ...
      state.desktopIconPositions[this.koba.id]={x:Math.round(this.koba.offsetLeft),y:Math.round(this.koba.offsetTop),overBuddyList:false};
      this.koba.classList.remove('over-buddy-list');
      saveState();
      this.scheduleSequence(function(){
        self.setState('Idle');
    ...
    function playKobaPostRunAdventure(runX,runY){
      ...
      kobaRoamEndTimer=setTimeout(function(){
        ...
        desktopCompanion.style.transition='none';
        desktopCompanion.style.left=runX+'px';desktopCompanion.style.top=runY+'px';
        ...
        requestAnimationFrame(function(){
          ...
          desktopCompanion.style.left=walkX+'px';desktopCompanion.style.top=walkY+'px';
          kobaRoamEndTimer=setTimeout(function(){playKobaSleepScene(walkX,walkY);},duration*1000+80);
        });
      },4900);
    }
    ...
    function startKobaRoam(skipWake){
      ...
      desktopCompanion.style.transition='none';
      desktopCompanion.style.left=startX+'px';desktopCompanion.style.top=startY+'px';desktopCompanion.style.right='auto';desktopCompanion.style.bottom='auto';
      ...
      requestAnimationFrame(function(){
        ...
        desktopCompanion.style.left=targetX+'px';desktopCompanion.style.top=targetY+'px';
        kobaRoamEndTimer=setTimeout(function(){finishKobaRoam(targetX,targetY,running);},duration*1000+80);
      });
    }
  ...
  function openOnlineDtdPostWindow(){
    ...
    createWindow({title:'DtD Post Mail',extraClass:'dtd-win',initialLeft:74,initialTop:72,bodyHtml:'<div class="win-body">'+bar()+'<div style="padding:28px;text-align:center">Connecting to DtD Post Mail…</div></div>',type:'dtdmail',onClose:function(){detachPigeonFromMailWindow();pigeonIcon().classList.remove('mail-window-open');if(pigeonComposingActive)endPigeonComposing(false);},onMount:function(el){
      ...
      function openMessage(id){
        ...
        main.querySelector('.dtd-back').onclick=renderRows;var reply=main.querySelector('.dtd-reply'),replyBox=main.querySelector('.dtd-inline-reply');reply.onclick=function(){replyBox.style.display='grid';reply.style.display='none';beginPigeonComposing();replyBox.querySelector('textarea').focus();};main.querySelector('.dtd-reply-cancel').onclick=function(){replyBox.style.display='none';reply.style.display='';endPigeonComposing(false);};main.querySelector('.dtd-reply-send').onclick=function(){var body=main.querySelector('.dtd-inline-reply-body').value,text=body.trim(),sendButton=main.querySelector('.dtd-reply-send');if(!text){main.querySelector('.dtd-reply-error').textContent='Write your reply first.';return;}sendButton.disabled=true;supabaseRpc('send_dtd_message',{recipient_handle:profileHandle,message_subject:m.message_subject||'',message_body:body,delivery_date:nextDeliveryDate(),client_timezone:dtdClientTimezone()}).then(function(){trackDtdUsage('letter_sent');endPigeonComposing(true);openInfoWindow('Your reply is sealed and will be delivered at the next 8:00am.');setFolder('sent');}).catch(function(err){main.querySelector('.dtd-reply-error').textContent=err.message;sendButton.disabled=false;});};
      }
