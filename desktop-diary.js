...
// DesktopDiary Paint v0.1 — self-contained application object.
  ...
  (function(){
    ...
    function defaultKobaKennelPoint(){
      ...
      var taskbarHeight=(taskbar&&taskbar.offsetHeight)||TASKBAR_H;
      return{x:Math.max(0,desktopWidth-width-8),y:Math.max(0,desktopHeight-taskbarHeight-height-6),overBuddyList:false,userPlaced:false};
    }
    function kobaKennelPoint(){
      ...
      if(!saved||!Number.isFinite(Number(saved.x))||!Number.isFinite(Number(saved.y))||(Number(saved.x)===0&&Number(saved.y)===0))return defaultKobaKennelPoint();
      var desktop=document.getElementById('desktop'),taskbar=document.getElementById('taskbar');
